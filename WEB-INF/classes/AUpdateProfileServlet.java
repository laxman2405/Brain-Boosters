...
public class AUpdateProfileServlet extends HttpServlet
{
	Connection cn;
	public void init()
	{
	...
	public void doGet(HttpServletRequest req,HttpServletResponse res) throws ServletException,IOException
	{
		...
		try
		{
			PreparedStatement ps=cn.prepareStatement("update Admin_reg set uname=?,pwd=?,fname=?,dateofbirth=?,gender=?,collegename=?,collegecode=?,gpa=?,rank=?,mobile=?,email=?,address=?,pincode=?  where uname=?");
			ps.setString(1,u);
			ps.setString(2,pwd);
			...
			out.println( "<center><h2><p><b>"+count+"Row is Updated");
			out.println("<h1><p><b>You have Successfully Updated your Account With New Details</h1><hr>");
			out.println("<div align='right'><form action='AdminPage.html' ><input type='submit' class='a' value='Log Out' height='40px' style='border:1px solid black;border-radius:50px;height:40px;width:150px;font-size:20px' ></form></div>");           
